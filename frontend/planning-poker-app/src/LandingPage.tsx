...
const LandingPage: React.FC = () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        localStorage.setItem(
          "ScrumMasterSession",
          JSON.stringify({ roomCode: data.RoomCode, isScrumMaster: true })
        );
  ...
  const handleJoinRoom = async () => {
    ...
    if (!roomCode || roomCode.length < 8) {
      setError((prev) => ({
        ...prev,
        roomCode: "Valid Room Code is required",
      }));
      return;
    }
    ...
    const response1 = await axios.get(
      "http://localhost:3000/check-room-status",
      { params: { roomCode } }
    );
    ...
    if (response1.data && !response1.data.isActive) {
      setError((prev) => ({
        ...prev,
        general:
          "This room has ended. Please contact the Scrum Master or create a new room.",
      }));
      return;
    }
    ...
    if (storedMemberId && storedMemberName === memberName) {
      ...
      navigate(`/room/${roomCode}`, {
        state: { memberId: storedMemberId, memberName },
      });
      ...
      try {
        const response = await axios.post(
          "http://localhost:3000/update-member-name",
          {
        ...
        if (response.data.success) {
          ...
          setError((prev) => ({
            ...prev,
            general: "Failed to update member name.",
          }));
        }
      } catch (error) {
        ...
        setError((prev) => ({
          ...prev,
          general: "Failed to update member name. Please try again.",
        }));
      }
    } else {
      try {
        ...
        if (data.success) {
          ...
          navigate(`/room/${roomCode}`, {
            state: { memberId: data.memberId, memberName },
          });
        } else {
          setError((prev) => ({
            ...prev,
            general: "Room not found. Please check the Room Code.",
          }));
        }
      } catch (error) {
        console.error("Error:", error);
  ...
  return (
    <div className="flex flex-col items-center justify-center h-screen bg-gray-100">
      <div className="bg-white shadow-lg rounded-lg p-8 max-w-md w-full">
        ...
        {error.general && (
          <p className="text-red-500 text-center mb-6">{error.general}</p>
        )}
        ...
        <div className="flex flex-col space-y-6">
          ...
          <div className="relative">
            ...
            />
            {error.roomCode && (
              <p className="text-red-500 text-sm mt-2">{error.roomCode}</p>
            )}
          </div>
          ...
          <div className="relative">
            ...
            />
            {error.memberName && (
              <p className="text-red-500 text-sm mt-2">{error.memberName}</p>
            )}
          </div>
